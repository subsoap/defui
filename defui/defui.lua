...
defui.action_id_touch       = hash("touch")
defui.action_id_text        = hash("text")
defui.action_id_marked_text = hash("marked_text")
defui.action_id_backspace   = hash("backspace")

defui.active_input_marked = ""
defui.active_node = nil
defui.input_nodes = {}
defui.colors = {
...
defui.utf8_gfind = "([%z\1-\127\194-\244][\128-\191]*)"


function defui.is_enabled(self, node)
    local parent = gui.get_parent(node)
    if parent then
        return defui.is_enabled(self, parent)
    end
...
local function hit_test(self, node, action_id, action)
    if not defui.is_enabled(self, node) then
        return false
    end

    local hit = gui.pick_node( node, action.x, action.y )
    local touch = action_id == defui.action_id_touch
    return touch and hit
end

function defui.hit(self, node, action_id, action, cb)
...
function defui.checkbox(self, node, action_id, action, value)
...
function defui.input(self, node, action_id, action, type, empty_text)
    ...
    if (not defui.input_nodes[key]) then
        defui.input_nodes[key] = { id = node, data = gui.get_text(node_content), active = false }
    end

    local input_node = defui.input_nodes[key]
    if not defui.is_enabled(self, node_bg) then
    ...
    if (not defui.active_node or defui.active_node ~= input_node) and input_node.active then
        ...
        gui.animate(active_node_bg, "color", defui.colors.base, gui.EASING_OUTCUBIC, 0.2)
    ...
    if hit_test(self, node_bg, action_id, action) then
            ...
            gui.reset_keyboard()
            defui.active_input_marked = ""
            defui.active_node = input_node
            defui.active_node.active = true
            gui.animate(node_bg, "color", defui.colors.active, gui.EASING_OUTCUBIC, 0.2)
    ...
    if (defui.active_node == input_node) then

        -- new raw text input
        if (action_id == defui.action_id_text) then
            defui.active_node.data = defui.active_node.data .. action.text
            defui.active_input_marked = ""

        -- new marked text input (uncommitted text)
        elseif (action_id == defui.action_id_marked_text) then
            defui.active_input_marked = action.text

        -- input deletion
        elseif (action_id == defui.action_id_backspace and (action.pressed or action.repeated)) then
            local last_s = 0
            for uchar in string.gfind(defui.active_node.data, defui.utf8_gfind) do
              last_s = string.len(uchar)
            end

            defui.active_node.data = string.sub(defui.active_node.data, 1, string.len(defui.active_node.data) - last_s)
        end

        -- set text color
        gui.set_color(node_content, defui.colors.enabled)

        -- if current input is active, include marked text in output
        text_output = defui.active_node.data .. defui.active_input_marked

        -- get text metrics for both raw input data and marked text
        local m_t = gui.get_text_metrics(gui.get_font(node_content), defui.active_node.data, 0, false, 0, 0)
        local m_m = gui.get_text_metrics(gui.get_font(node_content), defui.active_input_marked, 0, false, 0, 0)

        -- set cursor (and marked text bg)
        gui.set_position(node_cursor, vmath.vector3(4 + m_t.width, 0, 0))
        if (defui.active_input_marked and #defui.active_input_marked) then
    ...
    if (type == gui.KEYBOARD_TYPE_PASSWORD) then
        local masked_text = ""
        for uchar in string.gfind(text_output, defui.utf8_gfind) do
    ...
    if (empty_text and string.len(text_output) == 0) then
        gui.set_color(node_content, defui.colors.disabled)
...
function defui.scrollarea(self, node_str, action_id, action, scroll, cb)
